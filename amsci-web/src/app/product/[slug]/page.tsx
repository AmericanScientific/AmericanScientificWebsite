...
/**
 * Resolve a URL slug to the page to render.
 *
 * `product_groups.json` is a static artifact and the catalog is not: the cron
 * sync adds items to D1 whenever one is flipped online in NetSuite, and those
 * are absent from the grouping file until it is regenerated by hand. Falling
 * back to the catalog means a newly published product works immediately instead
 * of 404ing, while the grouping file stays authoritative wherever it has an
 * entry. Returns null only when nothing in either source matches.
 */
async function resolvePage(slug: string): Promise<VariantPage | null> {
	const grouped = pageBySlug(slug);
	if (grouped) return grouped;

	const product = await getProductBySku(slug);
	if (!product) return null;

	return syntheticPageForSku(product.sku, product.title, {
		internalId: Number(product.internalId) || 0,
...
export default async function ProductPage({
	...
	let page = pageBySlug(slug);

	if (!page) {
		// A legacy per-SKU URL (/product/3484-01) → 301 to the consolidated page,
		// deep-linking the member for multi-variant products. This must stay ahead
		// of the catalog fallback: a SKU that IS grouped belongs on its group page,
		// not on a synthesized one-off.
		...
		// Ungrouped: a product synced in after product_groups.json was generated.
		const synthesized = await resolvePage(slug);
		if (!synthesized) notFound();
		page = synthesized;
	}
